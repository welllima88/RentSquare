    ...
    $success = true;
		if(!empty($this->request->data)){
          ...
          if(empty($data['Unit']['rent']) || $data['Unit']['rent'] == ''){
    			...
    			if($data['Unit']['current_due_date'] == null || $data['Unit']['current_due_date']==""){ $data['Unit']['current_due_date'] = $data['Unit']['lease_start']; $this->__email_developer(array('Error'=>'Current Due Date Is Empty','Unit'=>$data['Unit']['id'],'Lease Start' => $data['Unit']['lease_start'])); }
    			$i=0;
  ...
  function deleteUnitFreeRents($unit_id = null)
     {
        ...
        if(!empty($unit_id))
        {
           ...
           $this->FreeRent->deleteAll(array('unit_id' => $unit_id,
                                            'billing_end >=' => $today), false, false);
           return 1;
        }  
        else 
        {
