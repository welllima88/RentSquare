...
<div class="unit_charges half_width">
  ...
  <h2 class="charge_type">Recurring Charges</h2>
    ...
    echo '<div class="form_input">';
          	...
          	echo $this->Form->input('Unit.rent', array('placeholder' => 'Rent','class'=>'dollar_input'));
          	echo '</div>';
        echo '</div><!-- .form_input -->';
  ...
  <div class="block_content">
    <div class="form_input">
      <?php echo $this->Form->hidden('Unit.prev_lease_start');
            if((string)$this->data['Unit']['lease_start'] == '01/01/1753'){
              $placeholder = 'N/A';
               $value = '';
            } else {
    ...
    <div class="form_input">
      <?php 
            if((string)$this->data['Unit']['lease_end'] == '01/01/1753'){
              $placeholder = 'N/A';
               $value = '';
            } else {
...
<div class="resident_info ">
  ...
  <table class="resident_info_edit_unit">
    ...
    <tr>
    	<td colspan="4" style="text-align:center; padding:10px 0; color:#c4c4c4;">
...
function free_rent(){
  ...
  $select_statement = '<div class="add_free_rent_item"><select name="data[Unit][99][free_rent]" class="UnitFreeRent"><option value="" selected="selected">Select Billing Period</option>';
  
  for(var i=0; i < $rent_range_options.length; i++){
    $select_statement = $select_statement + '<option value="' + $rent_range_options[i] + '">' + $rent_range_options[i] + '</option>';
  }
  $select_statement = $select_statement + '</select>';
  $select_statement = $select_statement + '<?php echo trim($this->Form->input('FreeRent.99.amount', array('placeholder' => 'Amount', 'class' => 'free_rent_amount' , 'label'=>false ,'div'=>false))); echo trim($this->Form->hidden('FreeRent.99.billing_start',array('class' => 'free_rent_start'))); echo trim($this->Form->hidden('FreeRent.99.billing_end',array('class' => 'free_rent_end'))); ?></div>';
