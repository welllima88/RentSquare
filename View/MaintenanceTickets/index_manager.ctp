...
<div class="block_title messages_block_title">
  <?php if(isset($curProp['name']) && $curProp['name'] !='') echo $curProp['name']; else echo 'Overview'; ?>
</div>
...
<div id='maintenance-overview' class='table messages'>
	<table class="table-striped format subheader">
		<tr class="no_click">
			<th><?php echo $this->Paginator->sort('Tenant.first_name','Name');?> <span class="caret"></span></th>
			<th width="45%"><?php echo $this->Paginator->sort('MaintenanceTicket.title','Subject');?> <span class="caret"></span></th>
		...
		<?php
		  if(isset($tickets) && count($tickets) > 0):
			...
			if(strlen($ticket['MaintenanceTicket']['title']) > 120){
  			  $subject = substr($ticket['MaintenanceTicket']['title'], 0, 120) . '...';
			  } else {
				...
				echo $this->Html->tableCells(array($ticket['Tenant']['first_name'] . ' ' .$ticket['Tenant']['last_name'] , $link, $unitNum, $this->Time2->messageTime($this->Time->convert(strtotime($ticket['MaintenanceTicket']['created']), $user_timezone)), array($status, array('class' => $status.' status'))), array('class' => 'odd'), array('class' => 'even'));
			}
			else:
