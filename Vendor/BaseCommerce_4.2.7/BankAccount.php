...
class BankAccount {
    ...
    static function buildFromJSON( $vo_json ) {
        ...
        if( array_key_exists( 'bank_account_deleted', $vo_json ) ) { $o_bank_account->setStatus("DELETED"); }
        
        if ( array_key_exists( "bank_account_cipher_pay_uuid", $vo_json ) ) {
            $o_bank_account->setCipherPayUUID( $vo_json[ "bank_account_cipher_pay_uuid" ] );
        }
    ...
    public function getJSON() {
        ...
        if ( !is_null( $this->is_cipher_pay_uuid ) ) {
            $o_json[ "bank_account_cipher_pay_uuid" ] = $this->is_cipher_pay_uuid;
        }
