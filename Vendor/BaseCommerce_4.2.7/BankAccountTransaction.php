...
class BankAccountTransaction {
    ...
    static $XS_BAT_STATUS_BATCHED = "BATCHED";
    ...
    private $is_check_number;
    private $is_individual_identification_number;
    ...
    public function setEffectiveDate( $vo_effective_date ) {
        ...
        if( PHP_VERSION < 5.3 ) {
            if( strpos($vo_effective_date, '/') !== FALSE ) {
                $this->io_effective_date = $this->date_create_from_format('m/d/Y H:i:s', $vo_effective_date )->format('m/d/Y H:i:s');
            } else {
                $this->io_effective_date = $this->date_create_from_format('m-d-Y H:i:s', $vo_effective_date )->format('m/d/Y H:i:s');
            }
        } else {
    ...
    public function setSettlementDate( $vo_settlement_date ) {        
        ...
        if( PHP_VERSION < 5.3 ) {
            if( strpos($vo_settlement_date, '/') !== FALSE ) {
                $this->io_settlement_date = $this->date_create_from_format('m/d/Y H:i:s', $vo_settlement_date )->format('m/d/Y H:i:s');
            } else {
                $this->io_settlement_date = $this->date_create_from_format('m-d-Y H:i:s', $vo_settlement_date )->format('m/d/Y H:i:s');
            }
        } else {
    ...
    public function setReturnDate( $vo_return_date ) {        
        ...
        if( PHP_VERSION < 5.3 ) {
           if( strpos($vo_return_date, '/') !== FALSE ) {
                $this->io_return_date = $this->date_create_from_format('m/d/Y H:i:s', $vo_return_date )->format('m/d/Y H:i:s');
            } else {
                $this->io_return_date = $this->date_create_from_format('m-d-Y H:i:s', $vo_return_date )->format('m/d/Y H:i:s');
            } 
        } else {
    ...
    /**
     * 
     * Returns the billing address of the bank account transaction
     * 
    ...
    /**
     * 
     * Sets the billing address of the bank account transaction
     * 
    ...
    public function getCheckNumber() {
        return $this->is_check_number;
    }
    
    /**
     * sets the check number
     * @param type $vs_check_number the check number
     * @author Rob Kurst <rob.kurst@basecommerce.com>
     */
    public function setCheckNumber( $vs_check_number ) {
        $this->is_check_number = $vs_check_number;
    }
    
    /**
     * returns the individual identification number
     * @return the individual identification number
     * @author Rob Kurst <rob.kurst@basecommerce.com>
     */
    public function getIndividualIdentificationNumber() {
        return $this->is_individual_identification_number;
    }
    
    /**
     * sets the individual identification number
     * @param type $vs_individual_identification_number the individual identification number
     * @author Rob Kurst <rob.kurst@basecommerce.com>
     */
    public function setIndividualIdentificationNumber( $vs_individual_identification_number ) {
        $this->is_individual_identification_number = $vs_individual_identification_number;
    }
    ...
    function getJSON() {
        ...
        if( !is_null($this->is_check_number ) ) {
            $o_json[ "bank_account_transaction_check_number" ] = $this->is_check_number;
        }
        
        if( !is_null($this->is_individual_identification_number ) ) {
            $o_json[ "bank_account_transaction_individual_identification_number" ] = $this->is_individual_identification_number;
        }
        
        if( !is_null($this->io_billing_address) ) {
            $o_json['bank_account_transaction_billing_address'] = $this->getBillingAddress()->getJSON();
        }

        if ( !is_null( $this->is_cipher_pay_uuid ) ) { $o_json[ "bank_account_transaction_cipher_pay_uuid" ] = $this->is_cipher_pay_uuid; }
