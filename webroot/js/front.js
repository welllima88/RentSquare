...
(function( $ ) {
  var progress = $('#progress'),
	...
	if(jQuery('#learn_more').length > 0){
		jQuery('#learn_more').click(function(e) {
			e.preventDefault();
			$('html,body').animate({ scrollTop: $("#rs_description").offset().top - 120}, 700, "swing");
		});
	}
	if(jQuery('#res_learn_more').length > 0){
		jQuery('#res_learn_more').click(function(e) {
			e.preventDefault();
			$('html,body').animate({ scrollTop: $("#res_description").offset().top - 70}, 700, "swing");
		});
	}
  if(jQuery('#learn_more_mobile').length > 0){
		jQuery('#learn_more_mobile').click(function(e) {
			e.preventDefault();
			$('html,body').animate({ scrollTop: $(".mobile_slider").offset().top - 70}, 700, "swing");
		});
	}
	...
	$(window).scroll(function() {    
	    ...
	    if (scroll >= 100) {
	        $("#fixed-menu").addClass("fixed-menu-show slideInDown");
	        /*
if(!$("#header-container").hasClass("small-menu")){
  	        $("#header-container").addClass("small-menu");
  	        $("#login-signup").stop().slideUp();
	        }
*/
	       
	    }
	    else {
	        $("#fixed-menu").removeClass("fixed-menu-show slideInDown");
	        /*
if($("#header-container").hasClass("small-menu")){
	          $("#header-container").removeClass("small-menu");
	          $("#login-signup").stop().slideDown();
	        }
*/
	    }
	});
	if($('li.click_nav').length > 0){
		$('li.click_nav').find('a').click(function(e) {
			e.preventDefault();
			$('html,body').animate({ scrollTop: $($(this).attr('href')).offset().top - 70}, 700, "swing");
		});
	}
	if($('.pay-now-popup').length > 0){
		$('.pay-now-popup').bind('inview', function (event, visible) {
		  if (visible == true) {
	...
	if($('.res_bill_popup').length > 0){
		$('.res_bill_popup').bind('inview', function (event, visible) {
		  if (visible == true) {
		    $(this).addClass('bounceIn');
		  } else {
		    $(this).removeClass('bounceIn');
	...
	//Price Slider on Property Manager Page
	pricing = $("#pricing-slider");
    monthlyPrice = $('#monthlyPricing');
    unitCount = $('#unitCount');
    unitsInput = $('.input-units');
	pricing.slider({
        ...
        create: function() {
            ...
            pricing.slider('value', 25);
            //unitsInput.val(1);
            //monthlyPrice.text(buildium.subscription.Plans[0].MonthlyPricing);
            //units.text(buildium.subscription.Plans[0].Units);
        },
        slide: function( event, ui ) {
	        $cur_val = ui.value;
	        $( "#input-units" ).val( $cur_val );
	        setPricing($cur_val);
	     }
    });
	jQuery('#').click(function(e) {
		e.preventDefault();
		pricing.slider('value', 1);
	});
	unitsInput.bind('keypress', function(e) {
        //if the letter is not digit greater than zero or comma, don't type anything
        return (!(e.which !== 8 && e.which !== 0 && e.which !== 44 && (e.which < 48 || e.which > 57)));
    });
	unitsInput.keyup(function() {
		unitsEntered = parseInt(this.value.replace(/,/g, ""), 10);
		pricing.slider('value', unitsEntered);
		setPricing(unitsEntered);
	});
	function setPricing($cur_val){
		if($cur_val <= 25){
	        	monthlyPrice.html('25');
	        	unitCount.html('25');
	        } else if($cur_val <= 50){
	        	monthlyPrice.html('50');
	        	unitCount.html('50');
	        } else if($cur_val <= 100){
	        	monthlyPrice.html('75');
	        	unitCount.html('100');
	        } else if($cur_val <= 200){
	        	monthlyPrice.html('125');
	        	unitCount.html('200');
	        } else if($cur_val <= 300){
	        	monthlyPrice.html('175');
	        	unitCount.html('300');
	        } else if($cur_val <= 400){
	        	monthlyPrice.html('225');
	        	unitCount.html('400');
	        } else if($cur_val > 400){
	        	monthlyPrice.html('275');
	        	unitCount.html('400+');
	        }
	}
})(jQuery);